...
class FlappyBird :
  ...
  def update(self) : 
    ...
    for pipe in self.pipes : 
      ...
      if self.agent.bird.x == pipe.x - 50 : 
        reward = 5
      ...
      if self.agent.bird.is_colliding(pipe) :
        reward, game_over = -10, True
        self.reset_game()
      
      if self.agent.bird.outside_borders() : 
        reward, game_over = -20, True
        self.reset_game()
    ...
    try : 
      ...
      while True : #game loop 
        ...
        if  current_time >= next_update or not self.slow_down :  #update game only every 'FPS' seconds
          ...
          if nb_loops >= 1 : 
            print('reward:', reward, '  action:', action, '  state:', current_state)
            if train :
              next_state = self.agent.get_game_state(self)
              self.agent.remember(current_state, action, reward, next_state, game_over)  
              self.agent.qtrainer.train(current_state, action,  
                                          reward, next_state, game_over) #train
