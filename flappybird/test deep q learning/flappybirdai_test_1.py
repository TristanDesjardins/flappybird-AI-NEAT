# -*- coding: utf-8 -*-
"""
Created on Sun Aug 21 14:12:58 2022
...
class FlappyBird :
  ...
  def update(self) : 
    ...
    for pipe in self.pipes : 
      ...
      if self.agent.bird.is_colliding(pipe) :
        reward, game_over = -1, True
        self.reset_game()
      
      if self.agent.bird.outside_borders() : 
        reward, game_over = -2, True
        self.reset_game()
  ...
  def train_agent(self):
    ...
    last_game = [] #tuples (state, action, reward, next_state) from last game on which we train at the end of the game
      ...
      while True : #game loop 
        ...
        if  current_time >= next_update or not self.slow_down :  #update game only every 'FPS' seconds
          ...
          if np.argmax(action) == 0 :
            if nb_loops - last_jump > 10 : #min nb loops between two jumps
              last_jump = nb_loops
          ...
          if train :
            ...
            last_game.append((current_state, action, reward, next_state, game_over))
          ...
          if game_over : 
            ...
            states, actions, rewards, next_states, game_overs = zip(*last_game)
            rewards = (reward,) * len(rewards)
            
            print(rewards)
            
            self.agent.qtrainer.train(states, actions, rewards, next_states, game_overs)       
